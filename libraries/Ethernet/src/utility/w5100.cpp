...
void  W5100Class::setSlot(uint8_t slot)
{
	
	const uint8_t slotAvalible[6]={1,3,5,7,9,11};
	
	bool slotOK = false; 
	for(uint8_t i=0;i<6;i++)
	{
		if(slotAvalible[i] == slot)
			slotOK = true;
	}
	if(!slotOK)
	{
		Serial.print("Error: Please use slot 1-6");
		return;
	}
	tonyCS_Sel = getPinIO[slot+1];
	
	Tony.pinMode(tonyCS_Sel, OUTPUT);
	Tony.digitalWrite(tonyCS_Sel, HIGH);
	//Tony.digitalWrite(tonyCS_Sel, LOW);
	//Tony.digitalWrite(tonyCS_Sel, HIGH);
...
W5100Linkstatus W5100Class::getLinkStatus_(void)
{
	uint8_t phystatus;
...
W5100Linkstatus W5100Class::getLinkStatus(void)
{
	  W5100Linkstatus ret;
	  Tony.digitalWrite(tonyCS_Sel, LOW);
	  ret = getLinkStatus_();
